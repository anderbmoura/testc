...
**Sempre** utilize os [tokens](?path=/docs/design-system-tokens-visão-geral--docs) e [temas](?path=/docs/design-system-temas-visão-geral--docs) definidos na configuração do Tamagui para manter a consistência visual.
...
> ⚠️ **Importante**: Consulte a documentação do [Sistema de Temas](?path=/docs/guia-de-contribuição-sistema-de-temas--docs) para entender como construir componentes que se adaptam corretamente aos diferentes temas da aplicação.
